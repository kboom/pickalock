 ...
 */
public class FixedSizeArray<T> extends AllocationGuard implements Iterable<T> {
    ...
    public final void addAll(FixedSizeArray<T> items) {
    	for(T t : items) {
    		if(!this.contains(t))
    			this.add(t);
    	}
    ...
    public void remove(T obj) {
    	final int index = find(obj, false);
        if (index != -1) {
            remove(index);
        }
    }
    ...
    public boolean contains(T obj) {
    	if(find(obj, false) != -1) return true;
    	else return false;
    }
    ...
    private StaticIterator iterator = iterator();
    public class StaticIterator implements Iterator<T>, Iterable<T> {

    	private int pos = 0;
    	
		@Override
		public boolean hasNext() {
			if(pos < mCount) return true;
			else {
				reset();
				return false;
			}
		...
		@Override
		public void remove() {
			FixedSizeArray.this.remove(pos);			
		}
	 ...
	 */
	public StaticIterator iterator() {		
		if(iterator == null) {
			iterator = new StaticIterator();
		} 
		return iterator;
	}
}
