...
public class TextureLibrary {
	...
	private static final int did = SLog.register(TextureLibrary.class);
	static {
		SLog.setTag(did, "Texture Library.");
	}
	
	
    TextureMapping[] mTextureHash;
    ...
    public TextureLibrary() {
        super();
        mTextureHash = new TextureMapping[DEFAULT_SIZE];
        for (int x = 0; x < mTextureHash.length; x++) {
            mTextureHash[x] = new TextureMapping();
        }
     ...
     * @return
     */
    public TextureMapping allocateTexture(int resourceID) {
        TextureMapping texture = getTextureByResource(resourceID);
        if (texture == null) {
    ...
    public TextureMapping loadTexture(Context context, GL10 gl, int resourceID) {
        TextureMapping texture = allocateTexture(resourceID);
        ...
        for (int x = 0; x < mTextureHash.length; x++) {
            if (mTextureHash[x].resource != -1 && mTextureHash[x].loaded) {
                ...
                if (error != GL10.GL_NO_ERROR) {
                    SLog.d(did, "Texture Delete, GLError: " + error + " (" + GLU.gluErrorString(error) + "): " + mTextureHash[x].resource);
                }
    ...
    protected TextureMapping loadBitmap(Context context, GL10 gl, TextureMapping texture) {
        assert gl != null;
        assert context != null;
        assert texture != null;
        if (texture.loaded == false && texture.resource != -1) {
            ...
            int error = gl.glGetError();
            if (error != GL10.GL_NO_ERROR) {
            	SLog.d(did, "Texture Load 1, GLError: " + error + " (" + GLU.gluErrorString(error) + "): " + texture.resource);
            }
            ...
            error = gl.glGetError();
            if (error != GL10.GL_NO_ERROR) {
            	SLog.d(did, "Texture Load 2, GLError: " + error + " (" + GLU.gluErrorString(error) + "): " + texture.resource);
            }
            ...
            error = gl.glGetError();
            if (error != GL10.GL_NO_ERROR) {
            	SLog.d(did, "Texture Load 3, GLError: " + error + " (" + GLU.gluErrorString(error) + "): " + texture.resource);
            }
            ...
            error = gl.glGetError();
            if (error != GL10.GL_NO_ERROR) {
            	SLog.d(did, "Texture Load 4, GLError: " + error + " (" + GLU.gluErrorString(error) + "): " + texture.resource);
            }
     ...
     */
    public TextureMapping getTextureByResource(int resourceID) {
        int index = getHashIndex(resourceID);
        int realIndex = findFirstKey(index, resourceID);
        TextureMapping texture = null;
        if (realIndex != -1) {
    ...
    protected TextureMapping addTexture(int id, int name, int width, int height) {
